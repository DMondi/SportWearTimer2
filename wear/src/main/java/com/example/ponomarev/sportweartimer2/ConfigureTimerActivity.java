...
public class ConfigureTimerActivity extends Activity {
    ...
    class onOkClick implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            data.putExtra("millis", MainActivity.getMilliseconds(ConfigureTimerActivity.this.hours, ConfigureTimerActivity.this.minutes, ConfigureTimerActivity.this.seconds));
            ConfigureTimerActivity.this.setResult(-1, data);
            ConfigureTimerActivity.this.finish();
        }
    }

    private class TimerModifier implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            ConfigureTimerActivity.this.updateTimerParts();
        }

        private long getValueForTimePart(int timePart) {
            if (timePart == TIME_PART_HOUR) {
                return ConfigureTimerActivity.this.hours;
            }
            if (timePart == TIME_PART_MINUTE) {
                return ConfigureTimerActivity.this.minutes;
            }
            if (timePart == TIME_PART_SECOND) {
                return ConfigureTimerActivity.this.seconds;
            }
            return 0;
        }
        ...
        private void setValueForTimePart(long theValue, int timePart) {
            if (timePart == TIME_PART_HOUR) {
                ConfigureTimerActivity.this.hours = theValue;
            }
            if (timePart == TIME_PART_MINUTE) {
                ConfigureTimerActivity.this.minutes = theValue;
            }
            if (timePart == TIME_PART_SECOND) {
                ConfigureTimerActivity.this.seconds = theValue;
            }
        }
    }
